...
const alice = async () => {
  ...
  const createTempTokenAccountIx = SystemProgram.createAccount({    //create temp account of alice token X------------------------
  ...
  const initTempAccountIx = Token.createInitAccountInstruction(   //set pub_key of alice to tempXTokenAccount data---------------
    TOKEN_PROGRAM_ID,
    XTokenMintPubkey,
    tempXTokenAccountKeypair.publicKey,
    aliceKeypair.publicKey
  );
  const transferXTokensToTempAccIx = Token.createTransferInstruction( //transefer 5X token to tempXTokenAccount----------------------------
    TOKEN_PROGRAM_ID,
  ...
  const createEscrowAccountIx = SystemProgram.createAccount({   //create escrow account for current escrow program----------------
  ...
  //init accounts such as alice, tempXTokenAccount, escrow, rent etc
  //run process_init_escrow in processor.rs
  //result - initialize esrow account -> 
      // is_initialized : true;
      // initializer_pubkey : alice pubkey
      // temp_token_account_pubkey : tempXTokenAccountKeypair.publicKey
      // initializer_token_to_receive_account_pubkey : aliceYTokenAccountPubkey
      // expected_amount : amount that Alice gives in escrow - 3X token
  const initEscrowIx = new TransactionInstruction({
    ...
    data: Buffer.from(
      Uint8Array.of(0, ...new BN(terms.aliceExpectedAmount).toArray("le", 8))   //3X token
    ),
  });
