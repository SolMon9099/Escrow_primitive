...
const bob = async () => {
  ...
  //read data from escrow account such as alice pub_key, 5X token, aliceTempXtokenAccount, aliceYtokenReceiveTokenaccount etc
  const escrowState = {
  ...
  //get PDA same as generate befor in process_init_escrow in processor.rs
  const PDA = await PublicKey.findProgramAddress(
    [Buffer.from("escrow")],
    escrowProgramId
  );
  //run process_exchange in processor.rs - exchange tokens
  const exchangeInstruction = new TransactionInstruction({
    programId: escrowProgramId,
    data: Buffer.from(
      Uint8Array.of(1, ...new BN(terms.bobExpectedAmount).toArray("le", 8))   //5Y token
    ),
    keys: [
